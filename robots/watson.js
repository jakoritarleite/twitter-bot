...
async function robot() {
	...
	async function classifyImage(index) {
		...
		try {
			const response = await visualRecognition.classify({ url: search.urls[index] });
			let score = 0
			let imageClasses = response.result.images[0].classifiers[0].classes.map((classes) => { return classes.class });
			let imageScores = response.result.images[0].classifiers[0].classes.map((classes) => { return classes.score });
